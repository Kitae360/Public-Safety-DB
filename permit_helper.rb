...
def insert_parking_permit_info()
	...
	p =  @help.question("What is the permit number?: ")
...
def update_parking_permit_info()
	...
	pn = @help.question("What is the parking permit number?")
	if @DB.check_exist_permit_with_pn(pn) == [[1]]
	y = @help.question("What is the type of the permit?: ")
	if HighLine.agree("Is this data correct? Permit Number: #{pn}, Type: #{y} (y/n)") == true
...
def search_permit_data()
	if @DB.check_permit_exist() == [[0]]
		...
		pn = @help.question("What is the permit number?: ")
		if @DB.check_permit_exist_pn(pn) == [[1]]
		@DB.select_with_pn(pn)
		else say("There is no permit with given permit number")
		end
	end
end
